...
export default function PaletteCard(props: PaletteCardProps) {
  ...
  return (
    <div className="relative">
      <Link
        to={`/app/palettes/${props.palette.id}`}
        aria-label={props.palette.name}
        className="flex flex-col w-full h-40 text-left truncate clickable-card"
      >
        {/* Title and More button */}
        <div className="p-2 mr-10">
          <h2 className="w-full overflow-hidden text-xl overflow-ellipsis">
            {props.palette.name}
          </h2>
        </div>
            ...
            if (shades.length >= 3) {
              ...
              styleObj = {
                background: `linear-gradient(to right, ${
                  shades[0]
                } 33.3%, ${middleShade} 0, ${middleShade} 66.6%, ${
                  shades[shades.length - 1]
                } 0)`,
              }
            } else if (shades.length === 2) {
              styleObj = {
                background: `linear-gradient(to right, ${shades[0]} 50%, ${
                  shades[shades.length - 1]
                } 50%)`,
              }
            } else {
      ...
      <TopRightIconButton
        ...
        aria-haspopup="menu"
        aria-expanded={Boolean(optionsAnchorEl)}
